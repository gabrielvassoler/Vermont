...
def gen_pkts(src_addr, dst_addr, src_port, dst_port, yfunc, lorem, seconds, msglen, hdslen, add_noise=True):
    ...
    while (x < 1):
        ...
        pkt = Ether() / IP(src=src_addr, dst=dst_addr, id=src_port) / \
...
def main():
    ...
    pkts = gen_pkts(src, dst, port, port, Yred, lorem, seconds, msglen, hdslen)
    #print('Writting traffic to pcap file', flush=True)
    if path['src'] not in senders.keys():
        senders[path['src']] = []
    senders[path['src']] += pkts
    ...
    for pair in path['cflows']:
        ...
        pkts = gen_pkts(ipsrc, ipdst, port, port, Yred, lorem, seconds, msglen, hdslen)

        if str(src) not in senders.keys():
            senders[str(src)] = []
        senders[str(src)] += pkts
    ...
    for k in senders:
        ...
        senders[k].sort(key=get_time)
        new_flow = {}
        new_flow['sender'] = 'h' + str(k)
        new_flow['sender_opt'] = ' --preload-pcap --stats=1 resources/workloads/general/host{}.pcp'.format(k)
        new_flow['sender_log'] = 'host{}-s.txt'.format(k)
        new_flow['start_delay'] = 0
        new_flow['duration'] = 1

        wl['tcpreplay'].append(new_flow)
        wrpcap('../../resources/workloads/general/host{}.pcp'.format(k), senders[k])
