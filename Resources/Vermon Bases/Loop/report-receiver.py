...
class PacketSniffer(threading.Thread):
    ...
    def log_packet(self, pkt):
        ...
        if Loop_Report in pkt:
            with open('results/'+str(c)+'result.json', 'a') as f:
                ax = str(current_mili()) + '\n'
                f.write(ax)
...
def main(n_nodes=5, username=None, hosts_per_node=2):
    global PROPERTIES
    global multiPathProperties
    global path_id_decoder
    #VERMON START
    ...
    hosts_per_node = int(hosts_per_node)
    ...
    for i in range(n_nodes):
        sniffers.append(PacketSniffer('s{}-eth{}'.format(i + 1, hosts_per_node + 1)))
...
if __name__ == '__main__':
    if len(sys.argv) != 4:
        print('Usage: report-receiver.py <n_nodes> <username> <hosts_per_node>')
    main(n_nodes=int(sys.argv[1]), username=sys.argv[2], hosts_per_node=sys.argv[3])
