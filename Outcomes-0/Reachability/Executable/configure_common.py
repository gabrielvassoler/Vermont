...
def main(net_file):
    ...
    for i in range(n_nodes):
        ...
        for j in range(hosts_per_switch):
            ...
            hosts[hid] = {
                ...
                'adj': nid,
                'commands': ["route add default gw 10.0.{}.{} dev eth0".format(node_num, 123),
                                "arp -i eth0 -s 10.0.{}.{} 08:00:00:00:{:02X}:00".format(node_num, 123,node_num)]
            }
            # add adjacency to hosts
            nodes[nid]['adj'].append(hid)
            all_links.append([hid, nid+"-p{}".format(nodes_ports[nid]), '0us', LBW])
            nodes_ports[nid] += 1
            # mac address
        ...
        all_links.append(['s0-p{}'.format(nodes_ports["s0"]), nid+"-p{}".format(nodes_ports[nid]), '0us', LBW])
        nodes_ports[nid] += 1
        nodes_ports["s0"] += 1
        table_entries[nid].append({
            "table": "egress.mac_addrs",
    ...
    for link in node_links:
        ...
        all_links.append([uid+"-p{}".format(nodes_ports[uid]), vid+"-p{}".format(nodes_ports[vid]), '0us', LBW])
        nodes_ports[uid] += 1
        nodes_ports[vid] += 1
    ...
    li = []
    for count in range(0, len(path['path']) - 1):
        li.append(['s'+str(path['path'][count]), _host, 's'+str(path['path'][count+1])])

    if path['loop'] == 1:
        li.append(['s'+str(path['path'][len(path['path']) - 1]), _host, 's'+str(path['path'][0])])
    ...
    dst = path['dst']
    port = path['port']
    path['cflows'].append([src,dst,port])
    ...
    for uid, hu in sorted(hosts.items(), key=lambda x: int(x[0][1:])):
        for vid, hv in sorted(hosts.items(), key=lambda x: int(x[0][1:])):
            if uid < vid:
                ...
                #VERMONT EXPERIMENTS - PORTS - They vary between 1234 and 2234 in a scenario with 1000 flows, these steps allow for different flow ids
                #for each flow generated in the network, despite same destination and origin
                for flow in path['cflows']:
                    if uid != 'h'+flow[0] or vid != 'h'+flow[1]:
                        continue
                    ######################
                    # FLOW: uid ===>>> vid
                    ######################
                    ...
                    table_entries[uadj].append({
                        ...
                        "match": {
                            ...
                            "hdrs.ipv4.identification": flow[2]
                        },
                ...
                #for each flow generated in the network, despite same destination and origin
                for flow in path['cflows']:
                    if uid != 'h'+flow[1] or vid != 'h'+flow[0]:
                        continue
                    ######################
                    # FLOW: uid <<<=== vid
                    ######################
    ...
    # GENERATE FILES
    all_links.sort(key=lambda x: (x[0][0], int(x[0].split("-")[0][1:]), int(x[1].split("-")[0][1:])))
    ...
    topology_cfg = {
        ...
        "hosts": hosts,
        "hosts_per_node": net['hosts_per_node'],
        "switches": {},
        "links": all_links
    }
