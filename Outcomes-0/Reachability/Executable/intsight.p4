...
#define REGWID 1024  // Number of registers to store flow statistics/metadata
...
control ingress(inout headers hdrs, inout custom_metadata_t cmd,
    ...
    table flow_ID {
        key = {
            ...
            hdrs.ipv4.identification: exact; //VERMONT PORTS
    ...
    apply {
        if(smd.egress_port != 0) {
            ...
            if(smd.instance_type == NORMAL_PACKET && !hdrs.report.isValid()
                    && !hdrs.arp.isValid() && !hdrs.loop_report.isValid()) {
                ////////////////////////////////////////////////////////////////
                ...
                if(hdrs.telemetry.isValid()) {
                    ...
                    if(hdrs.telemetry.loop_identifier == cmd.cmp && hdrs.telemetry.loop_detected == 0x0){
                        ...
                        clone_preserving_field_list(CloneType.E2E, INTSIGHT_MIRROR_SESSION, 0);
                    }else{
                      hdrs.telemetry.loop_identifier = cmd.cmp;
                    }
                }
