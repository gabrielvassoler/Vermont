...
class DisjointPath:
    ...
    def check(self, flow, decoder):
        #print(decoder.keys())
        path = decoder['s'+str(flow.startLocation)+',s'+str(flow.endLocation)+','+str(abs(flow.endLocation - flow.startLocation)+0)+',0']
    ...
    if(pathdst in PROPERTIES):
        for prop in PROPERTIES[pathdst]:
            ...
            if isinstance(prop, Waypoint):
                #print('wpdebug')
                #print(pathdst, pathsrc, flow)
                if pathdst == prop.endLocation and pathsrc == prop.startLocation and flow == prop.flowID:
                    prop.received_times.append(received_time)
                    path = path_id_decoder['s'+str(pathsrc)+',s'+str(pathdst)+','+str(pathLength)+',0']
                    ...
                    #print('WP - RECEIVED TIMES')
                    #print(prop.received_times)
                    #print('WP - PROCESS TIMES')
                    #print(prop.processing_times)
            ...
            if isinstance(prop, DisjointPathAux):
                if pathdst == prop.flow.endLocation and pathsrc == prop.flow.startLocation and flow == prop.flow.flowID:
                    ...
                    if f != "":
                        ...
                        #path = path_id_decoder['s'+str(pathsrc)+',s'+str(pathdst)+','+str(abs(pathdst - pathsrc)+0)+',0']
...
def final(PROPERTIES, multiPathProperties):
    ...
    c = 0
    with open('results/counter.txt', 'r+') as f:
        c = int(f.read(4))
        c += 1
        f.seek(0)
        f.write(str(c))

    with open('results/'+str(c)+'result.json', 'w') as f:
